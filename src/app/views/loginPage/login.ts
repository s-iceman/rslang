...
export class LoginView extends View implements IView {
  ...
  private autUserFormMetod(event: Event): void {
    ...
    if (userSignIn.checked) {
      ...
      const autorizedUser: IAutentificatedUser = { name: '' };
      signinUser.email = emailForm.value;
      signinUser.password = passwordForm.value;

      loginUser(signinUser)
        .then((response) => {
          const data = <IAutentificatedUser>response;
          autorizedUser.name = data.name;
          autorizedUser.refreshToken = data.refreshToken;
          autorizedUser.token = data.token;
          autorizedUser.userId = data.userId;
    ...
    if (userSignUp.checked) {
      const registrNewUser: INewUserRegistration = { isAuth: false, email: '', password: '', name: '' };
      if (name.value.toString() !== '') {
        ...
        registrNewUser.isAuth = true;
        createUser(registrNewUser)
          .then(() => {
            alert('Вы зарегистрированны');
            loginText.innerHTML = registrNewUser.name;
            localStorage.setItem('user', JSON.stringify(registrNewUser));
            window.location.href = '#';
          })
